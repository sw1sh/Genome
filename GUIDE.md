...
test specs (`*.wlt`), runner scripts (`*.wls`), and code in ` ```wl `
markdown cells. The rules at the top are non-negotiable; the rest are
conventions that keep code readable and consistent with the IDE
auto-formatter.
